...
# writes ema data to json file and filters it to start at jan 2 2002
def process_json_file(filename):
    ...
    filtered_time_series = {
        date: info for date, info in data["Technical Analysis: EMA"].items()
        if date >= date_threshold
    }

    transformed_data = {
        "name": data["Meta Data"]["1: Symbol"],
        "Technical Analysis: EMA": filtered_time_series
    }
...
def main(tickers: list):
    ...
    tickers = []
    tickers = [(ticker + "-ema.json") for ticker in tickers]
