...
# writes ochlv data to json file and filters it to start at jan 2 2002
def process_json_file(filename):
    ...
    filtered_time_series = {
        date: info for date, info in data["Time Series (Daily)"].items()
...
def main(tickers: list):
    ...
    tickers = []
    tickers = [(ticker + "-ochlv.json") for ticker in tickers]
